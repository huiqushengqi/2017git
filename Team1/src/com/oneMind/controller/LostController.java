...
public class LostController {
	@Resource
	private LostPostBiz biz;
	
	
	
     public LostPostBiz getBiz() {
		return biz;
	}



	public void setBiz(LostPostBiz biz) {
		this.biz = biz;
	}



	@RequestMapping(value="/Lostpost",method=RequestMethod.POST)
	public void addLost(String post_title,String post_depict,@RequestParam("post_file") MultipartFile[] files,HttpSession session,HttpServletResponse response){
		...
		if(biz.addLost(post)){
			out.print("<script>location.href='page.action'</script>");
		}else{
	...
	public String queryLostByPage(String page,HttpServletRequest request,HttpServletResponse response){
		...
		//根据当前页数和每页条数查询帖子
		List<Post> list=biz.queryLostByPage(thispage, size);
	...
	public void mar1(String postid,String content,String studentid,HttpServletResponse re){
	    ...
	    out.print("<script>location.href='market1.action?id="+id1+"'</script>");
		}
